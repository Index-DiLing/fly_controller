...
int main()
{
    ...
    if (mpu9250_dmp_init(
            ...
            MPU9250_ADDRESS_AD0_LOW, a_receive_callback, a_dmp_tap_callback, a_dmp_orient_callback) != 0) {
    ...
    uint16_t c = 32;
    while (true) {
        if (mpu9250_dmp_read_all(accel_raw, accel_g,
                                 gyro_raw, gyro_dps,
                                 gs_quat,
        ...
        for (uint8_t i = 0; i < len; i++) {
            ...
            //打印四元数
            logger<<"pitch: " << gs_pitch[i] << "roll: " << gs_roll[i]<< "yaw: " << gs_yaw[i]<< "\n";
            logger << LCMD::FLUSH;
        }
        len = 128;
        dl::delay_ms(200);
