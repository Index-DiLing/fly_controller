...
public:
    ...
    // 查看一个小端序数值
    template <typename T>
    bool peek_le(T& value) const {
        static_assert(std::is_trivially_copyable<T>::value,
                      "T must be trivially copyable");
        if (count < sizeof(T)) return false;
        uint8_t tmp[sizeof(T)];
        if (!peek(tmp, sizeof(T))) return false;  // 不会失败，安全起见检查
    ...
    // 查看一个大端序数值
    template <typename T>
    bool peek_be(T& value) const {
        static_assert(std::is_trivially_copyable<T>::value,
                      "T must be trivially copyable");
        if (!peek_le(value)) return false;
        // 对多字节类型交换字节序
        value = bswap(value);
        return true;
    }
...
private:
    ...
    public:
        ...
        ByteBuffer allocate(const uint16_t size)
        {
            ...
            current += size;
            return ByteBuffer(pool + current - size, size);
        }
    };
} // namespace dl
